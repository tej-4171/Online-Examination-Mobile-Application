        ...
        btn1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                while (currentUserIndex != userid.users.length) {
                    ...
                    if (s2.equals(userid.users[currentUserIndex])) {
                        currentUserIndex = 0;
                        break;
                    } else {
                        currentUserIndex++;
                ...
                if( s1.isEmpty() || s2.isEmpty() || s3.isEmpty() || s4.isEmpty() || s5.isEmpty() ){
                ...
                else if(s2.length()==10)
                 {
                    ...
                    fdb.child("QUser").addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            if(snapshot.hasChild(s2)){
                                ...
                                if( s4.equals(s5) ){
                                    fdb.child("QUser").child(s2).child("Name").setValue(s1);
                                    fdb.child("QUser").child(s2).child("Regno").setValue(s2);
                                    fdb.child("QUser").child(s2).child("Email").setValue(s3);
                                    fdb.child("QUser").child(s2).child("Password").setValue(s4);
                                    fdb.child("QUser").child(s2).child("Marks").setValue(-1);
                                    fdb.child("QUser").child(s2).child("Mobile").setValue("-");


                                    Toast.makeText(register.this, "details are stored", Toast.LENGTH_SHORT).show();
                    ...
                    Toast.makeText(register.this, "Enter valid Reg_no", Toast.LENGTH_SHORT).show();
                }
